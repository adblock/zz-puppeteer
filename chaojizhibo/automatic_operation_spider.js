    ...
    try {
        ...
        // 订阅 reponse 事件，参数是一个 reponse 实体
          await page.on('response', async (response) => {
            try {
              if (response.url().indexOf('https://adbrain.taobao.com/indexbp-live.html') !== -1) {
              ...
              if(response.url().indexOf('https://adbrain.taobao.com/api/member/getInfo.json') > -1) {
                let text = await response.text();
          ...
          await page.goto('https://adbrain.taobao.com/indexbp-live.html', {waitUntil: "networkidle0"});
          //若cookie 失效，或有滑块，或一直加载状态，开始下一个店铺
          if (page.url().indexOf('punish?x5secdata') > -1 || suberr === 0) {
...
const getCampaignCrowd = async(page, token, campaign_list) => {
    await asyncForEach(campaign_list, async(campaign) => {
        ...
        token = token.replace(/bizCode=(\S+)/, 'bizCode=adStrategyDuration');
        ...
        if (Object.keys(crowd_dict).length > 0) {
            ...
            let adgroup_data = await getCampaignAdgroup(page, token, campaign_id, adgroup_id);
            if(moment(start) <= moment() && moment() <= moment(end)){   // 在投放时间就 跑改价策略
                if(!(IS_RUNNING.hasOwnProperty(campaign_id) && IS_RUNNING[campaign_id] === 1)){ //首次开始改成最低出价并开启计划
                    await logPrint('开始爬取');
                    ...
                    await modifyPrice(page, token, campaign_id, need_price_crowd_dict);     // 改成最低出价
                    await logPrint('首次爬取，设置出价为最低出价(0.5)');

                    await modifyCampaignStatus(page, token, campaign_id, 'start');  // 开启计划
                    await logPrint('首次爬取，开启计划');
                }
                let need_price_crowd_dict = await strategyPrice(crowd_dict, campaign, adgroup_data);
                if(need_price_crowd_dict){  // 修改出价
                    await modifyPrice(page, token, campaign_id, need_price_crowd_dict);
                } else {    // 超过预算暂停计划
                    await modifyCampaignStatus(page, token, campaign_id, 'pause');
                    await logPrint('超过预算暂停计划');
                }
            } else {    // 暂停计划
                await modifyCampaignStatus(page, token, campaign_id, 'pause');
                await logPrint('投放结束，暂停计划');
            }
        } else {
...
const modifyPrice = async(page, token, campaign_id, need_price_crowd_dict) => {
    let url = 'https://adbrain.taobao.com/api/crowd/modifyEntireCrowd.json?' + token;
    let form_data = {
        'campaignId': campaign_id,
        'crowdList': JSON.stringify(Object.values(need_price_crowd_dict))
    };
    let resp = await sendPutRequest(page, form_data, url);
    console.log(resp);
};
 ...
 */
const strategyPrice = async(crowd_dict, campaign, adgroup_data) => {
    let crowd_list = adgroup_data['crowdList'];
    let need_price_crowd_dict = {};
    await asyncForEach(crowd_list, async(crowd) => {
        need_price_crowd_dict[crowd['crowdId']] = crowd;
    ...
    await asyncForEach(Object.keys(crowd_dict), async(crowd_key) => {
        ...
        let charge = crowd['reportInfoList'][0]['charge'];
    ...
    if(crowds_cost_now >= total_budget || charge_span >= budget){
        ...
        if(typeof average_cost !== "number"){
            average_cost = 0;
        }
        await logPrint('最小单位时间每个人群应该的花费(average_cost): ' + average_cost);
        // 判断 消耗 改价
        // 获取 该计划最近半个小时的 人群信息
        let crowd_data = await G_MONGO.db.collection('chaojizhibo.zhibo_crowd_data').find({'nick_name': wangwang,
        ...
        if(crowd_data.length > 0){
            ...
            await asyncForEach(Object.keys(crowd_data_recently), async(crowd_key) => {        // 1，先去比较最近一次的消耗和现在的消耗
                ...
                await logPrint(crowd['crowdName']);
                let charge_recently = crowd['reportInfoList'][0]['charge'];
                ...
                if(typeof charge_recently === "number"){
                    if(charge_dict[crowd_key] > charge_recently){       // 当前消耗比上一次消耗 变多了, 下一步判断 周期内花费是否花超（先简单假设，数据每次报存的都是最小单位的检查周期）
                        if(crowd_charge >= should_charge_now){     // 目前的消耗和理想中的消耗对比
                            ...
                            if(check_timer_charge > average_cost){      // 一个检查周期内花费超过了应该有的平均花费，降价 （正常的话价格不变）
                                let weight = check_timer_charge/average_cost;   // 超出 应该有的平均花费 多少倍
                                if(need_price_crowd_dict.hasOwnProperty(crowd_key)){
                                    let price_now = need_price_crowd_dict[crowd_key]['price'];
                                    let modify_price = price_now - price_now * 0.05 * weight;    // 百分之五*倍数 降价
                                    if(modify_price < 0.5){
                                        modify_price = 0.5
                                    }
                            ...
                            need_price_crowd_dict[crowd_key]['price'] = await addPriceWeight(crowd_charge, price_now, max_price, weight);
                        }
                        ...
                        if(crowd_charge < should_charge_now){     // 现在消耗小于理想消耗，进行权重加价
                            ...
                            if(crowd_data.length > n){      // 比较更早的一条，保证有两条数据，同理，如果要比较更早的n条，要有n+1 数据
                                for(let i=1; i<n+1; i++){
                                    ...
                                    if(typeof charge_recently_n === "number"){
                                        if(charge_dict[crowd_key] > charge_recently_n){     // 在选择的时间内消耗是有变化的，不需要加价了 （新增：要大于应该有的花费的1/2）
                                            if((charge_dict[crowd_key] - charge_recently_n) > average_cost/2){  // 如果这次单位时间内花费没有应该有的花费的1/2，也需要提高出价 todo
                                                is_modify = false;
                                            }
                                        }
                                    }
                                ...
                                if(is_modify){
                                    let price_now = need_price_crowd_dict[crowd_key]['price'];
                                    need_price_crowd_dict[crowd_key]['price'] = await addPriceWeight(crowd_charge, price_now, max_price);
                                }
                            }
                        }
                    }
...
/**
 * 根据权重加价
 * @param crowd_charge      当前的人群消耗
 * @param price_now         现在的出价
 * @param max_price         最高出价
 * @param weight            加倍权重
 * @returns {Promise<void>}
 */
const addPriceWeight = async(crowd_charge, price_now, max_price=1, weight=1) => {
    if(weight === Infinity){     // 默认是5倍（Infinity表示人群目前还是0花费）
        weight = 5;
    }
    console.log(price_now, weight);
    let modify_price = price_now + price_now * 0.05 * weight;   // 百分之五*权重加价
    if(modify_price > max_price && crowd_charge !== 0){   // 超过最高出价设为最高出价
        modify_price = max_price;
    } else if(modify_price > max_price * 2 && crowd_charge === 0){
         modify_price = max_price * 2;
    }
    await logPrint('现在的出价(price_now): ' + price_now);
    await logPrint('修改后的出价(modify_price): ' + modify_price);
    return modify_price;
 ...
 * @returns {Promise<*>}
 */
const getCampaignAdgroup = async(page, token, campaign_id, adgroup_id) => {
    let url = `https://adbrain.taobao.com/api/adgroup/get.json?&campaignId=${campaign_id}&adgroupId=${adgroup_id}` + token;
    const adgroup_resp = await sendRequest(page, url);
    return adgroup_resp['data']['adgroup'];
};
...
const saveCrowd = async(wangwang, save_data, campaign_id) => {
    ...
    await G_MONGO.db.collection('chaojizhibo.zhibo_crowd_data').deleteMany({'nick_name': wangwang, 'campaign_id': campaign_id, 'created_at': {'$lt': minutes_30_ago}});
    await G_MONGO.db.collection('chaojizhibo.zhibo_crowd_data').insertOne(data);
};

const getCrowdInfo = async(page, token, campaign_id) => {
    let today = moment().format('YYYY-MM-DD');
    let url = `https://adbrain.taobao.com/api/campaign/report/findCampiagnModeOverProductCrowdList.json?&queryUDF=false&logDateList=%5B%22${today}%22%5D&effect=15&effectType=click&adgroupId=&offset=0&pageSize=100&orderField=strategyId&orderBy=desc&campaignIdList=%5B%22${campaign_id}%22%5D` + token;
...
const modifyCampaignStatus = async(page, token, campaign_id, status) => {
    let update_url = 'https://adbrain.taobao.com/api/campaign/batchModify.json?' + token;
...
const getAutoShopByWangwang = async(wangwang) => {
    ...
    let sqls = "select * from t_automatic_operation_spider where f_type='chaozhi' and f_start <= '" + now + "%' and " +
    ...
    if(campaign_list.length > 0){
        // 获取总预算
        let sql_budget = `select f_budget from t_automatic_operation_spider where f_type='chaozhi' and f_wangwang='${wangwang}'`;
...
const sechdle = async(wangwang) =>{
    ...
    if(campaign_list.length > 0){
        ...
        try {
            ...
            console.log(e);
            await logPrint(wangwang + '改价失败' + e.toString() + '\n', 'error');
            if(browser){
                await browser.close();
            }
        }
...
const setLog = async(wangwang) => {
    ...
    log4js.configure({
        appenders:{
            out:{
                type: 'file',
                filename: config.auto_operation_path+'logs/' + today + '_' + wangwang + '.log'
            }
        },
        categories: { default: { appenders: ["out"], level: "info" }}
    });
    LOGGER = log4js.getLogger('chaojizhibo_automatic_operation_spider');
};
