...
const { ZhiboCampaignDataSpider } = require('./class/campaign_data_spider.class');
...
(async ()=>{
  ...
  // 店铺列表
  const shopList = await getCZZShopBoss('超级直播',[args[0],args[1]]);
  // 爬虫逻辑的实例
  const zhiboCampaignDataSpider = new ZhiboCampaignDataSpider({
    'crawlDate':crawlDate
  });
  // 爬虫公用逻辑实例
  const jupinSpider = new JupinSpider({
    'shopList':shopList,
    'spider': zhiboCampaignDataSpider
  });
  // 启动吧
