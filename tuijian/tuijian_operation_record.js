...
const startCrawl = async (shop, orgBrowser) => {
    try {
        ...
        if(page.url().indexOf('https://tuijian.taobao.com/indexbp-feedflow.html') > -1){
            if(token){
                save_data = await getData_Citj(page, token);
                if(huakuai === 0){
                    // 存储数据判断是否符合条件
...
/**
 * fetch url  超级推荐获取操作记录
 * @param page
 * @param token
 * @returns {Promise<void>}
 */
const getData_Citj = async(page, token) => {
    let refer = 'https://tuijian.taobao.com/indexbp-feedflow.html';
    ...
    for(let type in typeObj){
        ...
        const operation = await sendReauest(page, oper_url,refer);
...
const saveData  = async (wangwang, save_data) => {
    ...
    await G_MONGO.db.collection('chaojituijian_operation_record').deleteMany({'crawl_date': G_CRAWL_DATE, 'shop_name': wangwang});
    await G_MONGO.db.collection('chaojituijian_operation_record').insertOne(data);
    console.log(wangwang,'操作记录存储 ok');
};
...
const endCrawl = async function() {
    if(Object.keys(G_END_SHOP_HASH).length >= G_SHOP_LIST_ORG.length && G_SEND_STATUS){
        G_SEND_STATUS = false;
        const mail_url = config.php_url + 'cjtj/' + G_CRAWL_DATE;
 ...
 * */
const sendReauest = async (page,url,refer)=>{
    return await page.evaluate(async (url,refer) => {
        let headers = {
            'referer':refer,
            'origin':refer,
            'sec-fetch-mode':'cors',
...
(async () => {
    ...
    await G_MONGO.db.collection('chaojituijian_operation_record').deleteMany({'crawl_date': G_CRAWL_DATE});
