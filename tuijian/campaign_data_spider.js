...
const { TuijianCampaignDataSpider } = require('./class/campaign_data_spider.class');
...
(async ()=>{
  ...
  // 爬虫逻辑的实例
  const tuijianCampaignDataSpider = new TuijianCampaignDataSpider({
    'crawlDate':crawlDate
  });
  // 爬虫公用逻辑实例
  const jupinSpider = new JupinSpider({
    'shopList':shopList,
    'spider': tuijianCampaignDataSpider
  });
  // 启动吧
