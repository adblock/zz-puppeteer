...
let TYPE_CJZB = 'chaohu';      // 超级互动城
let PROCESS_DICT = {};          // 进程列表（每家店铺 对应一个进程）

const new_process = async(wangwang) => {
    console.log(wangwang, ': 开启进程');
    let workerProcess = child_process.fork(config['auto_operation_path_chaohu'] + 'automatic_operation_spider.js', [ wangwang ]);
...
const getAutoShops = async(type) => {
    ...
    if(shop_list.length === 0){
        ...
        await asyncForEach(shop_list, async(shop) => {
            ...
            let boss_sql = `select id from t_order where f_foreign_order_state_id=2 and f_foreign_sku_kind='超级互动城' and f_copy_wangwangid='${wangwang}';`;
