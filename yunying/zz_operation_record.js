/*
@File     ：zz_operation_record.py
@Author   ：qingyang
@Date     ：2021/9/28 09:47 
@describe ：运营钻展未操作记录
*/
...
const saveData = async(wangwang, save_data) => {
    ...
    await db.collection('yunying.zz_operation_record').deleteMany({$or:[{"shop_no_login":wangwang},{"shop_name":wangwang}], 'crawl_date':G_CRAWL_DATE});
    await db.collection('yunying.zz_operation_record').insertOne(data);
};
...
(async()=>{
    ...
    await sendMail('yy_zz', G_CRAWL_DATE);
})();