...
const { WanxiangtaiRealTimeSpider } = require('./class/wanxiangtai_report_realtime_spider.class');
...
(async ()=>{
  ...
  // 店铺列表
  const shopList = await getCZZShopBoss('万相台',[args[0],args[1]]);
  // 爬虫逻辑的实例
  const wxtRealTimeSpider = new WanxiangtaiRealTimeSpider({
    'crawlDate':crawlDate
  });
  // 爬虫公用逻辑实例
  const jupinSpider = new JupinSpider({
    'shopList':shopList,
    'spider': wxtRealTimeSpider
  });
  // 启动吧
