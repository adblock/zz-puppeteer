 ...
 * 万相台实时数据的爬虫逻辑类
 * */
class WanxiangtaiRealTimeSpider {
  constructor(option) {
  ...
  startCrawl = async () => {
    try {
      ...
      await this._page.on('response', async (response) => {
        try {
          if (response.url().indexOf('https://adbrain.taobao.com/indexbp.html') !== -1) {
            let text = await response.text();
            if (text.indexOf('_____tmd_____/punish') !== -1) {
              punish = true;                //在打开页面后判断true，执行进入下一个店铺命令，防止浏览器关闭报错
            }
          }
          // 万相台 nickname 得到text数据，校验店铺名
          if (response.url().indexOf('adbrain.taobao.com/api/member/getInfo.json') > -1) {
              getInfo = await response.text();
          }
          // 获取接口数据
          if (response.url().indexOf('findOverProductAccountRealTime.json') !== -1 && response.url().indexOf(this._crawlDate) >-1) {
            console.log(response.url());
            save_data.hourSum = await response.json();
          }
          if (response.url().indexOf('findOverProductAccountHourList.json') !== -1 && response.url().indexOf(this._crawlDate) >-1) {
            console.log(response.url());
      ...
      await this._page.goto('https://adbrain.taobao.com/indexbp.html', {waitUntil: "networkidle0"});
      ...
      if (
          this._page.url().indexOf('https://adbrain.taobao.com/index.html') > -1 ||
        ...
        if (Object.keys(save_data).length === 3 && insert === 0 && shop_name === 1) {
  ...
  saveData  = async (save_data) => {
    ...
    await this._mongo.db.collection('wanxiangtai.wxt_realtime_shop_data').deleteMany({
      'crawl_date': this._crawlDate,
      'nick_name': this._wangwang
    });
    await this._mongo.db.collection('wanxiangtai.wxt_realtime_shop_data').insertOne(data);
  };


}
module.exports = { WanxiangtaiRealTimeSpider };
