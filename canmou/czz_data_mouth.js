...
const startServer = async (account, crawl_date) => {
    ...
    const now = new Date();
    const mouth = dateFormat(await moment(crawl_date), 'm');
    const data = {'date': crawl_date, 'wangwang': account.wangwang_id, 'created_at': now, 'updated_at': now, 'mouth':mouth};
    ...
    await page.on('request', async(request)=>{
        const mouthBegin = moment(crawl_date).startOf('month').format("YYYY-MM-DD")
        if(request.url().indexOf('report/rptBpp4pCustomSum.htm') > -1){     // 拦截直通车接口，返回昨日数据
            const url = request.url().replace(/startDate=(\d{4}-\d{1,2}-\d{1,2})/, 'startDate='+mouthBegin).
            ...
            const url = request.url().replace(/startTime=(\d{4}-\d{1,2}-\d{1,2})/, 'startTime='+mouthBegin).
...
const saveData = async(data)=>{
    ...
    await db.collection('yunying_czz_data_mouth').deleteOne({'date': date, 'wangwang':wangwang});
    await db.collection('yunying_czz_data_mouth').insertOne(data);

    const conn = await connection(config.mysql_zhizuan);

    const mysql_data = {'f_shop': wangwang, 'f_date': date, 'created_at':now, 'updated_at':now, 'f_mouth':mouth};
    if('ztc' in data){
    ...
    // 更新/新增数据
    await conn.delete("t_yunying_czz_mouth").where("f_shop", wangwang).where('f_date', date).execute();
    const result = await conn.insert("t_yunying_czz_mouth", mysql_data).execute();
    console.log(result)
};
