...
const {CanmouIndexCrontabNew} = require('./class/canmou_index_crontab_new.class');
...
(async () => {
  ...
  // 爬虫逻辑的实例
  const canmouIndexCrontabNew = new CanmouIndexCrontabNew({
    'crawlDateArray': crawlDateArray
  });
  console.log('需要爬取的店铺个数',shopList.length);
  // 爬虫公用逻辑实例
  const jupinSpider = new JupinSpiderYunying({
    'shopList': shopList,
    'spider': canmouIndexCrontabNew
  });
  // 启动吧
...
const setShopListByDate = async (crawl_date) => {
  ...
  //获取已插入生意参谋日数据
  const sycm_index_sql = "select f_shop from t_sycm_index where f_insert_type = 1 and f_date='" + crawl_date + "'";
  ...
  let shop_lists = await getAllShopBossLiuliangYunying();
...
const setShopListByWangwang = async (wangwang, new_shop_lists, shop_lists, type) => {
  ...
  //获取已插入生意参谋日数据
  const sycm_index_sql = "select f_date from t_sycm_index where f_insert_type = 1 and f_date like'" + mouth + "%' and f_shop='" + wangwang + "'";
  const sycm_index = await mysqlCfgSql(config.mysql_zhizuan, sycm_index_sql);
  sycm_index.forEach((element, index) => {
