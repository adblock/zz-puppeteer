    ...
    try {
        ...
        await page.on('response', async (response) => {
            ...
            if (response.url().indexOf('getPersonalView.json') > -1) {
                ...
                const updateSql = 'update t_sycm_account set f_is_used = 1, f_valid_status = 1 , f_raw_cookies = \'' + JSON.stringify(saveCookies) + '\' , f_use_status = 0, updated_at ="' + update_at + '"  where id = ' + account.id;
            ...
            if (response.url().indexOf('https://login.taobao.com/newlogin/login.do') > -1) {
                ...
                if (data.content.data.redirectUrl === undefined) {
                    ...
                    if('密码'.indexOf(data.content.data.titleMsg) > -1){
                        const updateSql = 'update t_sycm_account set f_is_used = 3, f_valid_status = 0 , f_use_status = 0, updated_at ="' + update_at + '"  where id = ' + account.id;
                        await mysqlQuery(updateSql);
                    }
                    await browser.close();
                } else {
...
async function getSycmAccount(fId) {
    if(fId){
        ...
        let sql = 'select * from t_sycm_account';
    ...
    try {
        ...
        for (const account of accountList) {
            ...
            if (page.url() === homeUrl) {  // cookie有效，将有效状态保证为 1
                ...
                const update_at = dateformat(new Date(), "yyyy-mm-dd HH:MM:ss");
                const updateSql = 'update t_sycm_account set f_is_used = ' + isUsed + ', f_valid_status = 1 , f_raw_cookies = \''+ JSON.stringify(saveCookies) +'\' , f_use_status = 0, updated_at ="'+ update_at +'"  where id = '+ account.id;
                await mysqlQuery(updateSql);
            } else{
                // 将cookie有效状态改为0
