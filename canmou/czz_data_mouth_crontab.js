...
const { sendDingding } = require('../commons/func');
...
async function run_spider(shop_list, start, end, current_spider, result_spider, group_num, dingding_result){
    ...
    shop_list.forEach((element , index)=> {
        if(index  >= start && index  <= end){
            ...
            // 唤起爬虫
            ls = spawn('node', [config.canmou_login_user_data+'/czz_data_mouth.js', element.wangwang_id, crawl_date]);
...
async function getYunyingShop() {
    ...
    await asyncForEach(shop_lists, async (shop, index) => {
        const sql = "select count(id) from t_yunying_czz_mouth where f_date like'" + mouth + "%' and f_shop='" + shop.f_copy_wangwangid +"'";
