...
const { ZuanzhanPlangroupDataSpider } = require('./class/zz_plangroup_data_spider.class');
...
(async ()=>{
  ...
  // 爬虫逻辑的实例
  const zuanzhanPlangroupDataSpider = new ZuanzhanPlangroupDataSpider({
    'crawlDate':crawlDate
  });
  // 爬虫公用逻辑实例
  const jupinSpider = new JupinSpider({
    'shopList':shopList,
    'spider': zuanzhanPlangroupDataSpider
  });
  // 启动吧
