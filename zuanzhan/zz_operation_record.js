...
const { getCZZShopBoss, asyncForEach, setJs } = require('../commons/func');
...
const saveData = async(wangwang, save_data) => {
    ...
    await G_MONGO.db.collection('zuanzhan_operation_record').deleteMany({$or:[{"shop_no_login":wangwang},{"shop_name":wangwang}], 'crawl_date':G_CRAWL_DATE});
    await G_MONGO.db.collection('zuanzhan_operation_record').insertOne(data);
};
...
const sendMail = async() => {
    const mail_url = config.php_url + 'zz/' + G_CRAWL_DATE;
...
(async()=>{
    ...
    // 获取 服务中运营店铺的cookie
    let shop_on_service = await getCZZShopBoss(type='钻展');
